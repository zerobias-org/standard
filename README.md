# Standard mono-repo
Monorepo of standards.

# Starting Development
##  Fork Repository

Under the [zerobias-org/standard](https://github.com/zerobias-org/standard) repo, click the Fork button toward top right of the screen
* Under owner choose your personal account for development, and leave the repository name as standard
...
* Any PRs made against the `zerobias-org/standard` repo should be agaisnt it's dev branch or they will be rejected.
...
### Run create new standard script

### Install and Shrinkwrap

Run the following commands to update npm for your new standard
* `cd <folder_path>` cd into your new standard directory
* `npm install` run npm install
* `npm shrinkwrap` run npm shrinkwrap

### Validate new standards

In the root of the respository run the follow command to validate all edit or added standards
...
#### Standard versioning
Standard versions are managed by `lerna`. 
* Add it to [lerna.json](./lerna.json) to enable lerna for a standard.
* The starting version of a standard should be `0.0.0`
* There should be no version bumps inside of pull requests.

Lerna will automatically version, generate changelogs and publish standards in `lerna.json` via our github actions workflows.
...
In order for a standard/package to bump to `1.0.0` it must be explicitely told to do so via `premajor` and `conventional graduation`.
...
Each standard should have one workflow that runs tests on Pull Request.
...
#### Lerna Publish
This workflow will run lerna publish against all registered standards.
Standards that have changed are first bootstraped then tagged and published.
...
#### Lerna Post Publish
This workflow is used to finalize standard publication. At the moment it:
...
#### Build standard affected by change

The affected standard(s) will be built via `npm run build` and `unit` tested.
